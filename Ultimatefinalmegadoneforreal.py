...
def encode(image, message):
    ...
    #initializes the index that will be used to keep track of how much of the message has been encoded
    i = 0
...
#A WHOLE BUNCH OF IDIOT PROOFING
#***************************************************************

#Used to determine if there is an existing folder with a png file
def png(path):
    contains_png = False
    while not os.path.isdir(path):
        path = input("This is not an existing folder, enter another")
    for file in os.listdir(path):
        if file.endswith(".png"):
            return path
    return None

#initial check to see if there is an existing folder with a png file
folder_path = input("Enter the name of a folder containing a png file")
while not os.path.isdir(folder_path):
    folder_path = input("This is not an existing folder, enter another")

for file in os.listdir(folder_path):
    if file.endswith(".png"):
        i = 1
        print("Congrats! You managed to find a file with a png in it")
        break
    else:
        i = 0

while i != 1:
    folder_path = png(input("Enter the name of a folder containing a png file"))
    if folder_path is None:
        print("Your folder did not contain a png. Enter another")
    else:
        print("Congrats! You managed to find a file with a png in it")
        folder_path = png(folder_path)
        i = 1
#********************************************************************
#A WHOLE BUNCH OF IDIOT PROOFING
